...
{
	public void mergeFiles(int noOfIndices, String workingDir) throws IOException, InterruptedException
	{
		System.out.println("Index file merge program...");
		Process p;
		String s;
		BufferedReader br;
		String dir=workingDir+"/IndexFiles";
		...
		for(int i=1;i<=noOfIndices;i++)
		{
			...
			p.destroy();
			copyFilesToLocal(i, workingDir);
		}
		mergeFilesOnLocal(dir,noOfIndices);
		MainIndexTableInput mti =new MainIndexTableInput();
    	mti.createMainIndexFile(workingDir+"/IndexFiles/MergedOne", workingDir);
    	System.gc();
	}
	
	public void copyFilesToLocal(int i, String workingDir) throws InterruptedException
	{
		Process p;
		BufferedReader brb;
		String s;
		try {
			p=Runtime.getRuntime().exec("hadoop fs -copyToLocal /user/cloudera/Index_"+i+"/indexFile"+i+" "+workingDir+"/IndexFiles");
	...
	public void mergeFilesOnLocal(String dir, int noOfIndices) throws InterruptedException
	{
	    ...
	    for(int i=1;i<=noOfIndices;i++)
	    {
			...
			try
			{
				...
				File combineFile =new File(dir+"/MergedOne");
