...
public class PairWiseMatching 
{
    ...
    public void pairWiseMatching(String workingDir) throws ParserConfigurationException, SAXException, IOException, SQLException
    {
    	Calendar cal1=Calendar.getInstance();
        SimpleDateFormat sdf1 =new SimpleDateFormat("HH:mm:ss");
		String startTime =sdf1.format(cal1.getTime());
    	System.out.println("Pairwise matching of records program.....");
    	rulesMap = xmlrdr.ruleRead(workingDir+"/XmlFiles/CaptureRuleDescriptor.xml");
        ...
        while(res.next())
        {
            ...
            //System.out.println(counter++ +" "+dataKey +"  "+dataVal);
	        ...
	        for(Entry<String, String> cand: candidates.entrySet())
	        {
	        	...
	        	for (int i = 0; i < refids.length; i++)
	            {
	                for (int j = i + 1; j < refids.length; j++)
	                {
		                ...
		                if (trueCounter >= Math.ceil((float)rulesMap.size()/2))
		                {
		                    UID = randGenerator();
        ...
        BufferedWriter bw=new BufferedWriter(new FileWriter(workingDir+"/IndexFiles/PairOutput"));
        ...
        sql="LOAD DATA LOCAL INPATH '"+workingDir+"/IndexFiles/PairOutput' into table PAIROUTPUT";
	    ...
	    System.out.println("Total Matching and KB Insertion time! Start time: "+startTime+"  End Time: "+endTime);
        System.gc();
    }
